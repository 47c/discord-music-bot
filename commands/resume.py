...
@tree.command(
    name="resume",
    description="resumes current paused song",
    guild=discord.Object(id=1101666836690505819),
)
async def comm_resume(interaction: discord.Interaction):
    global guild_data
    ...
    await interaction.followup.send(content=data.audio_queue.resume())
